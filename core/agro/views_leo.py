...
from .models import TrazaLote, Lote, TrazaLoteItem, Mov, Com, Deposito, Contactos
...
def vista_trazabilidad_lote(request, id_estado):
    ...
    if estado.lote.campo.empresa == empresa:
        if request.method == 'POST':
            ...
            traza_lote_item_formset = TrazaLoteItemFormSet(prefix='traza_lote_item', initial=[{'moneda': empresa.moneda.id}])
        contexto = {
            'traza_lote_item_formset': traza_lote_item_formset, 
            'form': form, 
            'estado_lote': estado, 
            'empresa': empresa,
            'default_moneda': empresa.moneda,
            'default_cotizacion': 1,
            }            
        return render(request, 'vista_trazabilidad.html', contexto)
    else:
        return redirect('vista_lote_trazabilidad')
...
@login_required
def vista_resumen_trazabilidad(request, id_estado):
    try:
        estado = EstadoLote.objects.get(id = id_estado)
    except:
        return redirect('/')
    empresa = request.user.profile.empresa
    trazabilidades = TrazaLote.objects.filter(empresa=empresa).filter(estado_lote = estado)
    items = TrazaLoteItem.objects.filter(trazalote__estado_lote = estado)
    return render(request, 'vista_resumen_trazabilidad.html', {'trazas': trazabilidades, 'items': items, 'lote': estado.lote, 'campo': estado.lote.campo, 'estado': estado })
